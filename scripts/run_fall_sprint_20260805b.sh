...
PLAN="artifacts/20260806-034658-fall-sprint-daily-plan.json"
LOG="artifacts/20260806-fall-sprint-terminal.log"
...
./.venv/bin/python main.py check-linkedin-live 2>&1 | tee /tmp/fall-live-check-b.txt | tee -a "$LOG" | tail -20
if ! grep -qi 'session check passed' /tmp/fall-live-check-b.txt; then
...
(
  while true; do
    ...
    if ! curl -s --max-time 1 "http://127.0.0.1:${LINKEDIN_DEBUG_PORT}/json/version" >/dev/null; then
      echo "$(date -Iseconds) chrome down — relaunch" >>/tmp/outreach-chrome-watchdog-20260805b.log
      rm -f "$USER_DATA"/SingletonLock "$USER_DATA"/SingletonCookie "$USER_DATA"/SingletonSocket
      /Applications/Google\ Chrome.app/Contents/MacOS/Google\ Chrome \
