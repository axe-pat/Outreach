...
./.venv/bin/python main.py check-linkedin-live 2>&1 | tee /tmp/fall-live-check-20260818.txt | tee -a "$LOG" | tail -20
if ! grep -qi 'session check passed' /tmp/fall-live-check-20260818.txt; then
...
(
  while true; do
    ...
    if ! curl -s --max-time 1 "http://127.0.0.1:${LINKEDIN_DEBUG_PORT}/json/version" >/dev/null; then
      echo "$(date -Iseconds) chrome down — relaunch" >>/tmp/outreach-chrome-watchdog-20260818.log
      rm -f "$USER_DATA"/SingletonLock "$USER_DATA"/SingletonCookie "$USER_DATA"/SingletonSocket
      /Applications/Google\ Chrome.app/Contents/MacOS/Google\ Chrome \
...
set +e
caffeinate -dims ./.venv/bin/python -u scripts/send_approved_fall_invites_20260818.py \
  2>&1 | tee -a "$LOG"
RC=${PIPESTATUS[0]}
set -e
...
pkill -f "outreach-chrome-watchdog-20260818" 2>/dev/null || true
pkill -f "user-data-dir=${USER_DATA}.*--remote-debugging-port=${LINKEDIN_DEBUG_PORT}" 2>/dev/null || true
pkill -f "--user-data-dir=${USER_DATA} --remote-debugging-port=${LINKEDIN_DEBUG_PORT}" 2>/dev/null || true
pkill -f "caffeinate -dims ./.venv/bin/python -u scripts/send_approved_fall_invites_20260818.py" 2>/dev/null || true
