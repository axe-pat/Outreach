...
LOG="artifacts/20260807-followup-refresh.log"
...
# Keep Chrome alive if it dies mid-run
(
  while true; do
    sleep 45
    ...
    if ! curl -s --max-time 1 "http://127.0.0.1:${LINKEDIN_DEBUG_PORT}/json/version" >/dev/null; then
      echo "$(date -Iseconds) chrome down — relaunch"
      launch_chrome || true
      sleep 8
    fi
  done
...
./.venv/bin/python main.py check-linkedin-live 2>&1 | tee /tmp/fu-refresh-live-check.txt | tail -40
if ! grep -qi 'session check passed' /tmp/fu-refresh-live-check.txt; then
...
echo "==== PHASE 1: message reconcile (deep inbox) ===="
./.venv/bin/python main.py reconcile-linkedin-messages \
  --live \
  --deep \
  --include-seen \
  --apply \
...
echo "==== PHASE 2: build recent invited reconcile queue ===="
./.venv/bin/python main.py build-linkedin-reconcile-queue \
  --limit 200 \
  --max-age-days 7 \
  --min-age-hours 2
QUEUE="$(ls -t artifacts/*linkedin-reconcile-queue.json | head -1)"
echo "QUEUE=$QUEUE"

echo "==== PHASE 3: profile reconcile recent invited ===="
./.venv/bin/python main.py reconcile-linkedin \
  --live \
  --apply \
  ...
  --max-age-days 7 \
  --min-age-hours 2
echo "PHASE3_DONE=$(date -Iseconds)"

echo "==== PHASE 4: rebuild cadence + tiered backlog + drafts ===="
./.venv/bin/python <<'PY'
import csv
import json
import re
...
print("Building cadence report...", flush=True)
import subprocess
...
FOUNDER_EXEC = re.compile(
    r"\b(founder|co[- ]?founder|ceo|cto|cpo|coo|chief|vp\b|vice president|head of|founding)\b",
    re.I,
)
PM_OPS = re.compile(
    r"\b(product manager|\bpm\b|product lead|product ops|product operation|forward[- ]deployed|gtm|strategy|strategic|growth)\b",
    re.I,
)
ENGINEER_ONLY = re.compile(
    r"\b(software engineer|swe\b|sde\b|engineer|developer|sdet|embedded|ios |android |flutter|compiler)\b",
    re.I,
)
HIGH_ROLE = re.compile(
    r"\b(founder|co[- ]?founder|ceo|cto|cpo|coo|chief|vp|vice president|head of|director|product manager|pm\b|product lead|product ops|forward[- ]deployed|growth|talent|recruiting|people ops|hiring|gtm|strategy|strategic)\b",
    re.I,
)
MEH_ROLE = re.compile(
    r"\b(engineer|software|sde|swe|developer|designer|account exec|sales rep|recruiter coordinator|hr business|events|marketing coordinator|brand)\b",
    re.I,
)
...
def retier(r):
    ...
    if band in {"S_seed_proven"} and (founder or pm or not eng):
        return "T1_highest"
    ...
    if band in {"B_primary", "B_apply_target"} and score >= 35 and not eng:
        return "T2_strong"
    if band in {"B_primary", "B_apply_target", "C_verify_remote", "A_warm"} or r.get("band") == "fall_sprint":
        return "T3_solid"
    if founder or pm:
        return "T3_solid"
    return "T4_lower"
...
for r in due:
    ...
    rs = role_score(c.title, c.contact_type)
...
rows.sort(
    key=lambda x: (
        ...
        -float(x.get("fall_score") or 0),
        -int(x.get("role_score") or 0),
        x["company"],
        x["name"],
    )
)
...
print("Connected now:", connected, flush=True)
print("Backlog total:", len(rows), "by_tier", dict(Counter(r["tier"] for r in rows)), flush=True)
...
lines = [
    "# Follow-up backlog (tiered) — 2026-08-07 (LIVE REFRESH)",
    "",
    f"Total: **{len(rows)}** | Connected in tracker: **{connected}**",
    f"Cadence source: `{cadence_path}`",
    "",
    "## Counts",
    "",
]
    ...
    reconcile.append(
        {
            ...
            "thread_url": "",
            "detail": "Synthesized from live-refreshed cadence-due backlog",
            "state_reason": "cadence_due_backlog_live_refresh",
            "tier": r["tier"],
            "band": r.get("band"),
...
reconcile_path.write_text(
    json.dumps(
        {
            "created_at": datetime.now(timezone.utc).isoformat(),
            "source": "cadence_due_tiered_backlog_live_refresh",
            "count": len(reconcile),
            "results": reconcile,
        },
        indent=2,
...
print(f"Drafting {len(reconcile)} follow-ups with AI...", flush=True)
profile = load_style_profile_if_exists(
...
drafts.sort(
    key=lambda d: (
        tier_order.get(d.get("tier"), 9),
        -float(d.get("fall_score") or 0),
        d.get("company") or "",
        d.get("name") or "",
    )
)
...
md = [
    "# Follow-up draft pack — 2026-08-07 (LIVE REFRESH)",
    "",
    f"Total drafts: **{len(drafts)}**",
    f"By tier: {dict(Counter(d.get('tier') for d in drafts))}",
    "",
    "Generated after live LinkedIn reconcile. Engine still weak — review before send.",
    "",
]
for tier in tier_order:
...
print("Markdown written.", flush=True)
print("S_seed in backlog:", [r for r in rows if r.get("band") == "S_seed_proven"][:20], flush=True)
print(
    "T1 companies:",
    sorted({r["company"] for r in rows if r["tier"] == "T1_highest"}),
    flush=True,
)
PY
