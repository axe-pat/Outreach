...
LOG="artifacts/20260807-followup-refresh-continue.log"
...
launch_chrome() {
  ...
  /Applications/Google\ Chrome.app/Contents/MacOS/Google\ Chrome \
    ...
    --enable-automation --disable-extensions --no-first-run --disable-default-apps \
  ...
  for i in $(seq 1 40); do
    curl -s --max-time 2 "http://127.0.0.1:${LINKEDIN_DEBUG_PORT}/json/version" >/dev/null && {
      echo "CDP_READY at ${i}s"; return 0; }
    sleep 1
  done
...
./.venv/bin/python main.py check-linkedin-live 2>&1 | tee /tmp/fu-continue-live.txt | tail -20
if ! grep -qi 'session check passed' /tmp/fu-continue-live.txt; then
  echo LIVE_CHECK_FAILED; echo EXIT_CODE=3; echo DONE; exit 3
fi
echo LIVE_CHECK_OK

echo "==== PHASE A: focused Aug4+ fall Invited profile queue ===="
./.venv/bin/python <<'PY'
import csv, json
...
from outreach.cli import write_artifact
...
aug4 = datetime(2026, 8, 4, tzinfo=timezone.utc)
latest_invite = {}
for t in wb.list_touchpoints():
    ...
    if dt < aug4:
        continue
    cid = t.contact_id
    prev = latest_invite.get(cid)
    if prev is None or dt > prev[0]:
        latest_invite[cid] = (dt, t)

items = []
for cid, (dt, t) in latest_invite.items():
    c = contacts.get(cid)
    if not c or (c.status or "").lower() != "invited":
        continue
    ...
    ft = fall_for(org) or {}
    band = ft.get("band") or ""
    tags = ((org.target_lists if org else "") or "").lower()
    is_fall = "fall_sprint" in tags or band in BAND_RANK
    if not is_fall and band not in BAND_RANK:
        # still include recent Invited from Aug4+ invite runs (non-fall backfill)
        pass
    score = float(ft.get("fall_score") or 0)
    items.append({
        ...
        "band": band or ("fall_sprint" if "fall_sprint" in tags else "recent_invite"),
        "fall_score": score,
        "priority": BAND_RANK.get(band, 5 if is_fall else 6),
    })

items.sort(key=lambda x: (x["priority"], -x["fall_score"], x["company"], x["name"]))
# Cap at 90 focused profiles
items = items[:90]
artifact = write_artifact(
    OutreachSettings().artifacts_dir,
    "linkedin-reconcile-queue-fall-aug4",
    {
        "count": len(items),
        "filters": {"since": "2026-08-04", "status": ["Invited"], "prefer_fall": True, "limit": 90},
        "results": items,
    },
)
print(f"QUEUE={artifact}")
print(f"count={len(items)}")
from collections import Counter
print("bands", Counter(i["band"] for i in items).most_common())
print("companies", Counter(i["company"] for i in items).most_common(15))
PY

QUEUE="$(ls -t artifacts/*linkedin-reconcile-queue-fall-aug4.json | head -1)"
echo "QUEUE_FILE=$QUEUE"

echo "==== PHASE B: profile reconcile focused queue ===="
./.venv/bin/python main.py reconcile-linkedin \
  ...
  --max-age-days 14 \
  --min-age-hours 1
echo "PHASEB_DONE=$(date -Iseconds)"

echo "==== PHASE C: rebuild cadence + tiered backlog + drafts ===="
./.venv/bin/python <<'PY'
...
print("S_seed:", [(r["company"], r["name"]) for r in rows if r.get("band") == "S_seed_proven"], flush=True)
...
reconcile_path.write_text(json.dumps({"created_at": datetime.now(timezone.utc).isoformat(), "source": "live_refresh", "count": len(reconcile), "results": reconcile}, indent=2))

print(f"Drafting {len(reconcile)} follow-ups...", flush=True)
