    ...
    *,
    ...
    for org in orgs:
        if normalized_companies and _normalize_company(org.name) not in normalized_companies:
            continue
        reasons = company_context_gap_reasons(
            org,
...
def build_company_context_patch(
    ...
    *,
    ...
    if use_network:
        for url in _candidate_context_urls(org, fetcher=fetcher, use_web_search=use_web_search):
            html = _safe_fetch(fetcher, url)
            if not html:
                continue
            patch = _context_from_html(org.name, url, html)
            if patch and (patch.description or patch.tags or patch.website):
                return patch
    return _context_from_opportunities(opportunities)
...
def _search_company_urls(company: str, fetcher: HttpTextDownloader) -> list[str]:
    html = _safe_fetch(fetcher, f"https://duckduckgo.com/html/?q={quote_plus(company + ' company official website')}")
    if not html:
        return []
...
def _url_company_score(url: str, normalized_company: str) -> int:
    ...
    if not host or host in BLOCKED_SEARCH_DOMAINS:
        return 0
    if host.endswith("ycombinator.com") or "builtin" in host:
        return 2
